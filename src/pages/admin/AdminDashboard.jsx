  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-600 dark:text-slate-300">Content overview at a glance.</p>
      </div>

      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {stats.map((stat) => (
          <div key={stat.label} className="rounded-xl bg-brand-sky p-4 ring-1 ring-slate-200 dark:bg-slate-800 dark:ring-slate-700">
            ...
            <p className="mt-2 text-2xl font-black text-brand-navy dark:text-slate-100">{stat.value}</p>
          </div>
        ))}
      </div>
      ...
      <div className="rounded-xl border border-dashed border-slate-300 p-6 text-sm text-slate-600 dark:border-slate-700 dark:text-slate-300">
        ...
        <ul className="mt-2 list-disc space-y-1 pl-4">
          <li>Use Cloudinary uploads for thumbnails to get CDN + auto format.</li>
