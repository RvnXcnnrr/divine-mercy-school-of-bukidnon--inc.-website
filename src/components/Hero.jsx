...
export default function Hero() {
  ...
  const departments = useCountUp(2)
  const catholic = useCountUp(100)
  const faculty = useCountUp(20)
    ...
    >
      ...
      <div className="relative mx-auto grid max-w-7xl gap-14 px-4 pb-20 pt-16 lg:grid-cols-2 lg:items-center lg:py-24">
        <div className="relative z-10" data-reveal>
          ...
          <h1 className="page-h1 mt-4 max-w-xl">
            Faith-centered learning. Disciplined formation. Service with purpose.
          </h1>
          ...
          <div className="mt-8 flex flex-col gap-3 sm:flex-row">
            <NavLink to="/admissions" className="btn-primary btn-ripple">
              Enroll Now
        ...
        >
          ...
          <div
            ...
            style={{ transform: `translate3d(${mouse.x * -14}px, ${mouse.y * -12}px, 0)` }}
          >
            <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Departments</p>
            <p className="mt-1 text-2xl font-black text-brand-goldText">{departments}</p>
            <p className="text-xs text-slate-500">Junior and Senior High</p>
          </div>
          ...
          <div
            ...
            style={{ transform: `translate3d(${mouse.x * 16}px, ${mouse.y * 12}px, 0)` }}
          >
            <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Catholic Education</p>
            <p className="mt-1 text-2xl font-black text-brand-goldText">{catholic}%</p>
            <p className="text-xs text-slate-500">Values-based learning</p>
          </div>
          ...
          <div
            ...
            style={{ transform: `translate3d(${mouse.x * 11}px, ${mouse.y * -11}px, 0)` }}
          >
            <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Dedicated Faculty</p>
            <p className="mt-1 text-2xl font-black text-brand-goldText">{faculty}+</p>
            <p className="text-xs text-slate-500">Mentors and advisers</p>
          </div>
        </div>
      </div>
