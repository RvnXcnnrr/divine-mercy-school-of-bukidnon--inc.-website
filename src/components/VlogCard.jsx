...
export default function VlogCard({ item }) {
  ...
  return (
    <article className="flex flex-col overflow-hidden rounded-xl bg-white shadow-sm ring-1 ring-slate-200 transition-transform hover:-translate-y-0.5" data-reveal>
      ...
      <div className="flex flex-1 flex-col px-4 pb-4 pt-3">
        <div className="flex flex-wrap items-center gap-2 text-[11px] font-semibold uppercase tracking-wide text-brand-blue">
          {categoryLabel ? (
            <span className="inline-flex items-center gap-1 rounded-full bg-brand-sky px-2 py-1 ring-1 ring-slate-200">
        ...
        {item.description ? <p className="mt-1 text-sm text-slate-600 line-clamp-3">{item.description}</p> : null}
